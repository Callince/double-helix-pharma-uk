...
import { enquiries, timeAgo, type Enquiry, type EnquiryStatus } from "@/lib/admin/data";
...
const STATUS_OPTIONS: EnquiryStatus[] = ["new", "in_review", "contacted", "qualified", "won", "lost", "spam"];
...
export default function EnquiriesPage() {
  ...
  return (
    <>
      <PageHeader
        title="Enquiries"
        subtitle="Leads captured from the website contact form."
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                >
                  ...
                  <td className="px-5 py-3.5 text-xs text-muted">{timeAgo(e.createdAt)}</td>
...
function EnquiryDrawer({ enquiry, onClose }: { enquiry: Enquiry; onClose: () => void }) {
      ...
      <aside className="absolute inset-y-0 right-0 flex w-full max-w-md flex-col bg-white shadow-2xl">
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-6 py-5">
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <Detail label="Received" value={new Date(enquiry.createdAt).toLocaleString("en-GB")} />
          </div>
          ...
          <div>
            <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-muted">Add a note</p>
            <textarea
              rows={3}
              placeholder="Internal follow-up note…"
              className="w-full rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink outline-none placeholder:text-muted/60 focus:border-teal focus:ring-2 focus:ring-teal/25"
            />
          </div>
        </div>

        <div className="flex items-center gap-3 border-t border-line px-6 py-4">
          <AdminButton variant="green" icon="check" className="flex-1">Save changes</AdminButton>
...
function Detail({ label, value, link }: { label: string; value: string; link?: string }) {
  return (
    <div>
      ...
      {link ? (
        <a href={link} className="break-words text-teal-ink hover:underline">{value}</a>
      ) : (
