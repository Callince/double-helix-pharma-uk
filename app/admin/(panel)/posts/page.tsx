...
export default async function PostsAdminPage() {
  ...
  return (
    <>
      <PageHeader
        title="Insights"
        subtitle="Articles for the public blog — stored in SQLite."
        action={<AdminButton href="/admin/posts/new" icon="plus" variant="green">New insight</AdminButton>}
      />
      <Card pad={false} className="overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {posts.length === 0 ? (
                <tr><td colSpan={6} className="px-5 py-12 text-center text-muted">No insights yet.</td></tr>
              ) : posts.map((p) => (
                <tr key={p.id} className="border-b border-line/70 transition-colors last:border-0 hover:bg-surface/60">
                  <td className="px-5 py-3.5">
                    <Link href={`/admin/posts/${p.id}`} className="font-medium text-navy hover:text-teal-ink">{p.title}</Link>
                  </td>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center justify-end gap-1">
                      <Link href={`/admin/posts/${p.id}`} className="grid size-8 place-items-center rounded-lg text-muted hover:bg-surface hover:text-navy" aria-label="Edit">
